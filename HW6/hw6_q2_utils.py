...
def MincountLoss(output,boxes, mask,use_gpu=True):
    ones = torch.ones(1)
    ...
    if boxes.shape[1] > 1:
        ...
        for tempBoxes in boxes.squeeze():
            ...
            #mask = resize(mask)
            #print(mask.size())
            mask_ = mask[y1:y2, x1:x2].cuda()
            
            #print(output[:,:,y1:y2,x1:x2].size(),mask_.size())
            X = (output[:,:,y1:y2,x1:x2] * (mask_)).sum()
            if X.item() <= 1:
                Loss += F.mse_loss(X,torch.ones(X.size()).cuda())
    else:
        boxes = boxes.squeeze()
        y1 = int(boxes[1])
        y2 = int(boxes[3])
